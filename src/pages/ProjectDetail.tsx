...
interface Project {
  ...
  images?: string[]; videos?: string[]; documents?: string[]; document_labels?: string[]
  status: string; category: string; skill_codes?: string[]
}
...
export default function ProjectDetail() {
  ...
  if (!project) return (
    <div className="min-h-screen bg-surface-light dark:bg-surface-dark flex flex-col items-center justify-center gap-4">
      ...
      <Link to="/#projects" className="text-orange-500 hover:underline text-sm">← Retour aux projets</Link>
    </div>
  )
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'dark' : ''} bg-surface-light dark:bg-surface-dark`}>
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="mb-8">
          ...
          <h1 className="font-head font-black text-3xl md:text-5xl
                  ...
                  {images.map((img, i) => (
                    <button key={i} onClick={() => setImgIndex(i)}
                      className={`flex-shrink-0 w-20 h-14 rounded-xl overflow-hidden border-2 transition-all
                        ${i === imgIndex ? 'border-orange-500' : 'border-border-light dark:border-border-dark opacity-50 hover:opacity-100'}`}>
        ...
        {project.tech.length > 0 && (
          <div className="mb-8">
            <h2 className="text-xs uppercase tracking-widest text-muted font-semibold mb-3
          ...
          <div className="mb-8">
            ...
            <div className="space-y-4">
              {videos.map((v, i) => {
                ...
                return ytId ? (
                  <div key={i} className="w-full aspect-video rounded-2xl overflow-hidden
                    border border-border-light dark:border-border-dark">
              ...
              {documents.map((doc, i) => (
                <a key={i} href={doc} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl
                    ...
                    border border-blue-700/20 hover:bg-blue-700/20 transition-colors text-sm">
                  <FileText size={14} />
          ...
          {project.demo_url && (
            <a href={project.demo_url} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-full
                ...
                transition-all duration-200 shadow-lg shadow-orange-500/25 hover:-translate-y-0.5">
